...
export default function GameRoomPage() {
  ...
  useEffect(() => {
    ...
    const cleanup = socket.onUserJoined((data) => {
      setPlayers((prev) => {
        ...
        return [
          ...prev,
          { userId: data.userId, displayName: data.displayName, score: 0 },
        ];
      });
    });
    return cleanup;
  ...
  const handleSelectOption = (optionId: string, correct: boolean) => {
    ...
    if (correct) {
      ...
      setScore(newScore);
      setPlayers((prev) =>
        prev.map((p) => (p.userId === user.id ? { ...p, score: newScore } : p)),
      );
    }
        ...
        <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full p-4 sm:p-6 lg:p-8">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-3 h-3 rounded-full ${timeLeft > 10 ? "bg-green-400" : timeLeft > 5 ? "bg-yellow-400" : "bg-red-400 animate-pulse"}`}
              />
